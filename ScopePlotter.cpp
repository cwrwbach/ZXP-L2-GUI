...
//  setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    ...
    m_Percent2DScreen = 100;	//FINDME  //percent of screen used for 2D display
...
//Cursor line
painter.setPen(QPen(QColor(0xFf,0xff,0xf,0xff), 1, Qt::DotLine));
x=250; y=500;
painter.drawLine(x, 0, x, y);
update();
...
printf(" line 360 \n");
 ...
 update();
    ...
    // Frequency grid
    qint64  StartFreq  = 5505000; //= m_CenterFreq + m_FftCenter - m_Span / 2;
...
float horiz_Pixperdiv;
  // My extra draw vertical grid lines
    horiz_Pixperdiv = 20; //(float)plot_width / (float)plot_HorDivs;
    y = 500; //plot_height ; 
